...
import (
	...
	"mars-mec-go/event"
	"mars-mec-go/log"
	"mars-mec-go/profiler"
	"mars-mec-go/rpc"
	marsSync "mars-mec-go/util/sync"
	"mars-mec-go/util/timer"
)
...
var eventPool = marsSync.NewPoolEx(make(chan marsSync.IPoolData, maxServiceEventChannel), func() marsSync.IPoolData {
	return &event.Event{}
})
...
func SetMaxServiceChannel(maxEventChannel int) {
	...
	eventPool = marsSync.NewPoolEx(make(chan marsSync.IPoolData, maxServiceEventChannel), func() marsSync.IPoolData {
		return &event.Event{}
	})
}
